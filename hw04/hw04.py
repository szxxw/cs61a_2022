...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    if is_leaf(t) and label(t) == "loki":    
        ...
        my_branches = [replace_loki_at_leaf(b,lokis_replacement) for b in branches(t)]
        return tree(label(t), my_branches)
...
def sub_interval(x, y):
    ...
    and any value in y."""
    "*** YOUR CODE HERE ***"
    new = interval(-upper_bound(y), -lower_bound(y))
    return  add_interval(x, new)
...
def div_interval(x, y):
    ...
    assert upper_bound(y) and lower_bound(y) != 0, 'interval y should not contains zero'
    assert 1 / upper_bound(y) < 1/lower_bound(y), 'Error'
