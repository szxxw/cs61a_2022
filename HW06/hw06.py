...
def is_bst(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    # return the minimum value of a tree's left entries
    def bst_min(t):
        if t.is_leaf():
            return t.label
        return min(t.label, bst_min(t.branches[0]))
    # return the maximum value of a tree's right entries
    def bst_max(t):
        if t.is_leaf():
            return t.label
    ...
    if len(t.branches) == 1:
        c = t.branches[0]
        return is_bst(c) # and (bst_min(c) > t.label or bst_max(c) <= t.label)
    elif len(t.branches) == 2:
        c1, c2 = t.branches[0],t.branches[1]
        # For every node, the entries in that node's left child are less than or equal to the label of the node, For every node, the entries in that node's right child are greater than the label of the node
