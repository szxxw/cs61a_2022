...
class Player:
    ...
    def play(self, index):
        ...
        """
        "*** YOUR CODE HERE ***"
        assert index < len(self.hand), 'index is out of the list'
...
class TutorCard(Card):
    ...
    # at a cost of automatically loses the current round
    def power(self, opponent_card):
        return -float('inf')
...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        #             if max < player.hand[i+1].power(opponent_card):
        #                 max = player.hand[i+1].power(opponent_card)
        #                 index = i+1
        #     a = player.hand.pop(index)
