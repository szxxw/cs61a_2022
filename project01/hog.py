...
def tail_points(opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    assert type(opponent_score) == int and opponent_score >= 0, 'Opponent_score must be a non-negative integer.'
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    def always_roll_n(score1, score2):
        return n
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for score1 in range(goal):
        for score2 in range(goal):
            rolls.add(strategy(score1, score2))
    return len(rolls) == 1
...
def make_averaged(original_function, total_samples=1000):
    ...
    >>> averaged_dice = make_averaged(roll_dice, 40), roll_dice(num_rolls, function_of_dice)
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    return_num_rolls = 0
    for i in range(1,11):
        result = make_averaged(roll_dice, total_samples)
        outcome = result(i,dice)
        #print(outcome)
        if outcome > bigest_value:
            bigest_value = outcome
            return_num_rolls = i
    return return_num_rolls
    # END PROBLEM 9
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    increase_score = tail_points(opponent_score)
    if increase_score > threshold:
        return 0
    elif perfect_square(increase_score + score):  # Implement perfect_square
        if next_perfect_square(increase_score + score) - score > threshold:
            return 0
        else:
            return num_rolls
    else:
