...
def sprout_leaves(t, leaves):
    ...
    """
    "*** YOUR CODE HERE ***"
    # turn new leaves to tree data abstraction
    my_list = [tree(i) for i in leaves]
    def helper(t):
      # if it is leaf, add new leaves to it
      if is_leaf(t):
        #print(tree(label(t),my_list))
        return tree(label(t),my_list)
      else:
        bs = [helper(b) for b in branches(t)]
        # remember to excute this step
...
def preorder(t):
    ...
    # def helper(t):
    #   for i in branches(t):
    #     my_list.append(label(i))
    #     if is_leaf(i) is False:
    #       helper(i)
    #   return my_list
    # return helper(t)
...
def add_trees(t1, t2):
    ...
    for j in range(min_len, max_len):
      if len1 < len2:
        new_branches.append(branches(t2)[j])
      else:
        new_branches.append(branches(t1)[j])
